...
struct Extensor {
    ...
    pub fn run(&mut self, ebnf: String) -> String {
        let mut out = self.reset();
        for line in ebnf.lines().map(str::trim) {
            if line.is_empty() || line.starts_with("//") {continue}
            let [rule, productions] = line.splitn(2, "->").map(|part| part.trim()).collect::<Vec<&str>>()[0..2] else {panic!("{line}")};
            let body = self.expand(productions, &mut out);
    ...
    fn postfix(&mut self, expression: &str, rules: &mut HashSet<String>) -> String {
        ...
        while let Some(hit) = POSTFIXREGEX.captures(&result.as_bytes()) {
            ...
            rules.insert(match operator.as_str() {
