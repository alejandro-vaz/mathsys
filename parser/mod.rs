...
pub(super) struct Parser {} impl Parser {
    ...
    ) -> &'exists mut FastSet<SmallVec<[Backpointer<'parsing>; MINPOINTERS]>> {
        return chart[index as usize].entry(state.clone()).or_default()
    }
    #[inline(always)]
    fn review<'exists, 'parsing>(
        ...
        state: &State, 
        chart: &'exists mut Vec<FastMap<State, FastSet<SmallVec<[Backpointer<'parsing>; MINPOINTERS]>>>>
    ) -> &'exists FastSet<SmallVec<[Backpointer<'parsing>; MINPOINTERS]>> {
        return chart[index as usize].entry(state.clone()).or_default()
    }
    pub(super) fn run<'parsing>(
        ...
        let root = State::new(&arena, Rule::Internal(0), 0, 0, 0);
        ...
        for index in 0..(tokens.len() + 1) {
            ...
            while !agenda.is_empty() {
                ...
                if state.at().is_none() {
                    self.complete(index as u32, state, &mut agenda, &mut completed, &mut pool, &mut waiting, &mut chart, &arena);
                } else if let (Some(Symbol::Kind(kind)), Some(value)) = (state.at(), token.cloned()) && kind == value.kind {
                    self.scan(index as u32, state, value, &mut waiting, &mut chart, &arena);
                } else {
                    self.predict(index as u32, state, &mut agenda, &mut waiting, &mut chart, &arena);
                }
            }
            completed.clear();
        };
    ...
    fn complete<'parsing>(
        ...
        for awaiting in waiting[state.starting as usize].get(&state.rule.clone().into()).cloned().unwrap_or_default() {
            ...
            if additional.len() > stored {
                agenda.push_back(advanced.clone());
